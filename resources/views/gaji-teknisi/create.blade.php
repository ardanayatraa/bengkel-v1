<x-app-layout>
    <div class="mx-auto sm:px-6 lg:px-8 py-6">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-lg font-semibold">Tambah Gaji Teknisi</h2>
        ...
        <div class="bg-white shadow rounded-lg p-6">
            <form action="{{ route('gaji-teknisi.store') }}" method="POST">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <select name="id_teknisi" id="id_teknisi" required 
                            ...
                            @foreach($teknisis as $teknisi)
                                <option value="{{ $teknisi->id_teknisi }}" 
                                        {{ old('id_teknisi') == $teknisi->id_teknisi ? 'selected' : '' }}>
                        ...
                        <select name="id_jasa" id="id_jasa" required 
                            ...
                            @foreach($jasas as $jasa)
                                <option value="{{ $jasa->id_jasa }}" 
                                        ...
                                        {{ old('id_jasa') == $jasa->id_jasa ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="number" name="harga_jasa" id="harga_jasa" 
                               value="{{ old('harga_jasa') }}" required min="0"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                    ...
                    <div>
                        ...
                        <input type="number" name="persentase_gaji" id="persentase_gaji" 
                               value="{{ old('persentase_gaji') }}" required min="0" max="100" step="0.01"
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500"
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal_kerja" id="tanggal_kerja" 
                               value="{{ old('tanggal_kerja', date('Y-m-d')) }}" required
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-red-500">
                        @error('tanggal_kerja')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
